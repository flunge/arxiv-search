...
def fix_captions(content: str) -> str:
    ...
    for i, m in enumerate(caps, 1):
        ...
        if len(plain) < 12:
            ...
            plain = strip_tags(txt)
        if i <= 2 and len(plain) < 36:
            txt = (txt.rstrip("。") + "。并说明关键模块、输入输出与结论对应关系。").strip()
...
def main() -> None:
    ...
    ap.add_argument("category", choices=["sanitize", "captions", "templates", "technical", "equations", "takeaway"])
    args = ap.parse_args()
